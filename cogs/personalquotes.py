...
class PersonalQuotes(commands.Cog):
    ...
    @personal.command(**get_command_kwargs(get_cog_name(), "quote"))
    async def quote(self, ctx, quote: Option(str, autocomplete=get_quote, **get_parameter_kwargs(get_cog_name(), "quote.quote"))):
    ...
    @personal.command(**get_command_kwargs(get_cog_name(), "clear"))
    ...
    @personal.command(**get_command_kwargs(get_cog_name(), "code"))
    async def code(self, ctx, quote: Option(str, autocomplete=get_quote, **get_parameter_kwargs(get_cog_name(), "code.quote"))):
        user_quote = await GG.MDB['personalcommands'].find_one({"user": ctx.interaction.user.id, "trigger": quote})
        replaceString = '\`'
        await ctx.respond(f"```{user_quote['response'].replace('`', replaceString)}```", files=user_quote['attachments'])

    @personal.command(**get_command_kwargs(get_cog_name(), "add"))
    async def add(self, ctx,
                  quote: Option(str, **get_parameter_kwargs(get_cog_name(), "add.quote")),
                  response: Option(str, **get_parameter_kwargs(get_cog_name(), "add.response")),
                  attachment: Option(discord.Attachment, required=False, **get_parameter_kwargs(get_cog_name(), "add.attachment"))):
    ...
    @personal.command(**get_command_kwargs(get_cog_name(), "delete"))
    async def delete(self, ctx, quote: Option(str, autocomplete=get_quote, **get_parameter_kwargs(get_cog_name(), "delete.quote"))):
    ...
    @personal.command(**get_command_kwargs(get_cog_name(), "list"))
